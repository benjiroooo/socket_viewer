...
const BACKGROUND_COLOR_DARK = 'rgb(26, 26, 26)';
...
function init() {
  ...
  graphicStats.domElement.style.position = 'relative'; // Change from absolute
  graphicStats.domElement.style.left = 'auto'; // Reset positioning
  graphicStats.domElement.style.top = 'auto'; // Reset positioning
  graphicStats.domElement.className = 'stats'; // Add class for CSS targeting
  ...
  trackStats.domElement.style.position = 'relative'; // Change from absolute
  trackStats.domElement.style.left = 'auto'; // Reset positioning
  trackStats.domElement.style.top = 'auto'; // Reset positioning
  trackStats.domElement.className = 'stats'; // Add class for CSS targeting
...
function initGui() {
  ...
  // Add Dark Mode checkbox at the top

  gui
    .add(property, 'CameraMode', ['Above', 'Follow', 'Bird', 'Subjective'])
    .onChange(setCameraMode);
  gui
    .add(property, 'DarkMode')
    .name('Dark Mode')
    .onChange(function (value) {
      const newTheme = value ? 'dark' : 'light';
      document.body.setAttribute('data-theme', newTheme);
      localStorage.setItem('theme', newTheme);
      updateRendererBackground(value);
    });
  ...
  gui
    .add(property, 'CurrentFrameSize', 0, 4, 0.1)
    .onChange(setCurrentframeSize);
  ...
  gui.add(property, 'ResetSignal').domElement.children[0].innerHTML =
    "<button onclick='onClickReset()'>reset</button>";
  gui.add(property, 'StopSignal').domElement.children[0].innerHTML =
...
function updateMapElements(
  ...
  if (receiveTimestamp != 0) {
    ...
    console.log(
      ('         ' + parseInt(msgSize / 1000)).substr(-6) +
        ' KB' +
        ('     ' + fps.toFixed(1)).substr(-7) +
        ' fps, ' +
        ('         ' + pointCloud.nValidPoint).substr(-6) +
        ' pts, ' +
        ('         ' + cameraFrames.numValidKeyframe).substr(-6) +
        ' kfs'
    );
  }
...
function onThumbClick() {
  ...
  if (!thumbEnlarge) {
    ...
    document.getElementById('thumb').style.transform =
      'translate(' + x + 'px, ' + y + 'px) scale(' + THUMB_SCALING + ')';
  }
}

// Add this function to update renderer background
function updateRendererBackground(isDark) {
  if (renderer) {
    if (isDark) {
      renderer.setClearColor(new THREE.Color('rgb(26, 26, 26)'));
    } else {
...
window.updateRendererBackground = updateRendererBackground;
