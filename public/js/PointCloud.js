...
class PointCloud {
  ...
  addPoint(id, x, y, z, r, g, b) {
    // Convert black points to white in dark mode
    if (property.DarkMode && r === 0 && g === 0 && b === 0) {
      r = 255;
      g = 255;
      b = 255;
    }
    ...
    if (this.discardedPoolSize > 0) {
      ...
      if (
        this.cloudGeometries[cloudIdx].vertices[pointIdxInCloud] === undefined
      ) {
      ...
      this.cloudGeometries[cloudIdx].colors[pointIdxInCloud] = new THREE.Color(
        'rgb(' + r + ',' + g + ',' + b + ')'
      );
      ...
      this.cloudGeometries[cloudIdx].colors.push(
  ...
  changePointColor(id, r, g, b) {
    ...
    this.cloudGeometries[cloudIdx].colors[vertexIdxOnCloud] = new THREE.Color(
      'rgb(' + r + ',' + g + ',' + b + ')'
    );
  ...
  removePoint(id) {
    ...
    this.changePointPos(
      id,
      this.POOL_POINT_COORDS[0],
      this.POOL_POINT_COORDS[1],
      this.POOL_POINT_COORDS[2]
    );
  ...
  colorizeReferencePoints(referencePointIds) {
    for (let id of referencePointIds) {
      pointCloud.changePointColor(
        id,
        REFERENCE_POINT_COLOR[0],
        REFERENCE_POINT_COLOR[1],
        REFERENCE_POINT_COLOR[2]
      );
  ...
  updatePointInScene(scene) {
    for (let i = 0; i < this.cloudGeometries.length; i++) {
      ...
      try {
        ...
        this.clouds[i] = new THREE.Points(
          this.cloudGeometryCaches[i],
          this.cloudMaterial
        );
